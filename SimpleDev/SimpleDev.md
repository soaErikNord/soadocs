...
```
Create the file ‘widget_factory.js’ in the following folder structure (../resources/theme/developer/SOA/Dev/extensions), if it doesn’t already exist:
 
Add the following widget code to the file:
```json
"footer" : {
    "widgets" : [{
        "position" : "html",
        "view" : "DEMO/Dev/footer/views/footer.ejs"
    }]
}
```
